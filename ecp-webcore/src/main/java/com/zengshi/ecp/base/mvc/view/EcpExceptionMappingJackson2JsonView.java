...
package com.zengshi.ecp.base.mvc.view;
 ...
 */
public class EcpExceptionMappingJackson2JsonView extends AbstractView {

    private static final Logger logger=Logger.getLogger(EcpExceptionMappingJackson2JsonView.class);
	/**
	 ...
	 */
	public EcpExceptionMappingJackson2JsonView() {
		setContentType(DEFAULT_CONTENT_TYPE);
		setExposePathVariables(false);
	}

	public EcpExceptionMappingJackson2JsonView(Exception ex) {
	    super();
	    this.exception=ex;
	}
	/**
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
        ...
        boolean flag=false;
	    if(JsonResultThreadLocal.get()){
	        ResponseModel jsonObj=new ResponseModel();
	        jsonObj.setAjaxResult(ResultTypeEnum.ERROR);
	        jsonObj.setErrorMessage(new ArrayList<MessageModel>());
	        
	        if(BindException.class.isAssignableFrom(this.exception.getClass())){
	            BindException bindEx=(BindException)this.exception;
	            List<ObjectError> objErrs= bindEx.getAllErrors();
	            for(ObjectError objErr : objErrs){//org.springframework.context.support.DefaultMessageSourceResolvable
	                String msg=objErr.getDefaultMessage();
	                if(StringUtils.hasText(msg)){
	                    Object[] args=objErr.getArguments();
	                    MessageModel msgModel=new MessageModel(objErr.getObjectName(),msg);
	                    for(Object arg : args){
	                        if(null!=arg && DefaultMessageSourceResolvable.class.isAssignableFrom(arg.getClass())){
	                            DefaultMessageSourceResolvable dsr=(DefaultMessageSourceResolvable)arg;
	                            msgModel.setCode(dsr.getCode());
	                            break;
	                        }
	                    }
	                    jsonObj.getErrorMessage().add(msgModel);
	                }
	            }
	            jsonObj.setAjaxResult(ResultTypeEnum.INVALID);
	        }else if(BusinessException.class.isAssignableFrom(this.exception.getClass())){
	            BusinessException busExp=(BusinessException)this.exception;
	            jsonObj.getErrorMessage().add(new MessageModel(busExp.getErrorCode(),busExp.getErrorMessage()));
	        }else{
	            jsonObj.getErrorMessage().add(new MessageModel(null,this.exception.getMessage()));
	            flag=true;
	        }
	        jsonObj.setValues(value);
	        this.objectMapper.writeValue(generator, jsonObj);
	    }else{
        ...
        if(flag){
            logger.error("throws exception", this.exception);
        }
	}
