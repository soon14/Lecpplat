 ...
 * 功能更新
 * 2016.07.22 重构eAjax
 * 2016.10.10 修改信息内容读取规则，支持单一错误信息文本，而非一定要数组对象
 * 
 */
;(function ($) {
	"use strict";
				...
				switch (data.ajaxResult) {
					case 0://访问失败、后台异常、业务异常等非正常结果
						...
						if(data.errorMessage){
							if($.isArray(data.errorMessage) && data.errorMessage.length > 0){
								$.each(data.errorMessage, function(i, row) {
									errmsg.push(ebcUtils.htmlEncode(row.message));
								});
							}else if($.type(data.errorMessage) == 'string'){
								errmsg.push(ebcUtils.htmlEncode(data.errorMessage));
							}
						}
					...
					case 2://后台校验信息
						...
						break;
					case 3://登录超时
						eDialog.alert('会话超时，请重新登录!', function() {
							window.top.location.replace($webroot + 'login');
						}, 'error');
						break;
					default:
						break;
