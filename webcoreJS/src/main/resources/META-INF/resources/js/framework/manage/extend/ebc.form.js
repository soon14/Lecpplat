...
var ebcForm = {
			...
			if($.isArray(data) && data.length>0){//数组格式数据（后台数据格式：List<xxx>）
				...
				objs.each(function(i,row){
					...
					$.each(data,function(index,dRow){
						keyStr = dRow[opts.keyField];
						valueStr = dRow[opts.valueField];
	...
	formParams : function(p) {
		...
		var putParam = function (_obj,_dataList){
			if($.isArray(_dataList) && _obj && $(_obj).attr("name")){
				_dataList.push({ 'name' : $(_obj).attr("name"),'value' : $.trim($(_obj).val()) });
			}
		};
		...
		$(p).find(":input").each(function(i, n) {
			...
			if (flag == true || flag == "true") {
				if ($(n).attr('type') == "text" || $(n).attr('type') == "password" || $(n).attr('type') == "hidden" || n.tagName == 'TEXTAREA') {
					...
					if ($.trim($(n).val()) != '') putParam(n,_pdata);
					else {
						if($(n).attr("name")){
		...
		var putParamOnly = function(name,value,arr){
			if($.isArray(arr) && name){
				var setValue = function(){
					arr.push({'name' : name, 'value' : $.trim(value)});
				};
				if(arr.length>0){
	...
	/**
	 * 在表单中的任意可输入控件中输入回车执行表单查询
	 * 
	 * @param {string|object} [form] - 表单对象的jquery选择器表达式，例如：'#searchForm','.searchForm'等，若不传递该参数，则会使用'#searchForm'的表达式进行查询
	 * @param {string|object} [btn] - 提交表单数据的按钮jquery选择器表达式，若不传递该参数，则会使用'#btnFormSearch'的表达式进行查询
	 * 
	 * @remark 若使用form、btn两个参数都找不到实际的元素，则该函数不执行实际功能
	 */
	enterSubmitForm : function(form,btn){
		var defaultForm = '#searchForm',defaultBtn = '#btnFormSearch';
		var _form = $(form).size()==0 ? $(defaultForm) : $(form);
		
		if($(_form).size() == 0){
			//console.warn('ebcForm.enterSubmitForm：表单表达式设置不正确，未找到对应的表单元素');
			return;
		}
		var _btn = $(btn).size()==0 ? $(defaultBtn) : $(btn);
		if($(_btn).size() == 0){
			//console.warn('ebcForm.enterSubmitForm：提交表单数据按钮表达式设置不正确，未找到对应的按钮元素！或可将提交表单按钮id属性设置为btnFormSearch则可不需要传递参数2');
			return;
		}
		
		$(':text,select',$(_form)).keypress(function(e){
			var key_code = window.event ? window.event.keyCode : e.which ? e.which : e.charCode;
			if(key_code==13){
				e.stopPropagation();
				$(_btn).click();
				return false;
