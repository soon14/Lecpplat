 ...
 * @version 7.4.4
 * @author Yuusaku Miyazaki <toumin.m7@gmail.com>(宮崎 雄策)
 * @license MIT License
 * 
 * 二次开发 by Terry
 * 
 * 
 * 修改内容：
 * 修复设置的初始值在列表中不存在时，报错的情况
 * 修改输入框右边的下拉面板为Bootstrap样式，并移到输入框内部
 * 增加在ajax查询数据的情况下，使用自定义参数进行数据查询
 * 修改使用ajax方式查询，直接适配Grid查询的数据格式
 * 增加简体中文语言支持
 * 
 * 2016.04.20
...
;(function($){
	var constants = {
		dataKey : 'ajaxComboBoxObject',                     //插件缓存内部对象的KEY
		objStatusKey : 'ajaxComboBox-self-mark',            //全局范围设置当前点击是否为插件自身的标识
		objStatusIndex : 'ajaxComboBox-self-index'          //全局范围设置当前点击的ajaxcombobox的索引下标
	};
	/**
	...
	/** @lends AjaxComboBox.prototype */
	$.extend(AjaxComboBox.prototype,{
		/**
		 * @private
		 * @desc オプションの初期化
		...
		_setTagOptions: function(option, idx) {
			...
			// order_byを配列にする
			option.tags[idx].order_by = (option.tags[idx].order_by === undefined) ? option.order_by: this._setOrderbyOption(option.tags[idx].order_by, option.tags[idx].field);
		...
		_setElem: function(combo_input) {
			...
			if (this.option.plugin_type == 'textarea') elem.hidden = false;
			else {
				/**
				...
				elem.hidden = $('<input type="hidden" class="ac_hidden" />').attr({
					name: hidden_name,
					id: hidden_id
				}).val('');
			...
			case 'combobox':
				...
				break;
			case 'simple':
				$(elem.container).append(elem.result_area).append(elem.hidden);
				break;
			case 'textarea':
				$(elem.container).append(elem.result_area);
			}
			$(elem.result_area).append(elem.results).append(elem.navi).append(elem.sub_info);

			//将所有分页信息合并为一行
			//$(elem.navi).append(elem.navi_info).append(elem.navi_p);
			// 3. サイズ調整
			// ComboBoxの幅
			//下拉触发按钮现在已经整合到输入框内部，不再是扩展在输入框外部的部分，所以不再需要进行宽度设置
			/*
		    if (this.option.plugin_type == 'combobox') {
		...
		_afterInit: function(self, data) {
			...
			if (self.option.select_only) { // 選択状態
		...
		_ehWhole: function() {
			...
			$(self.elem.container).mousedown(function() {
				var thisindex = $('div.ac_container').index(this);
			...
			//控件外部的鼠标点击事件处理
			$(document).off('mousedown.ajax-combobox').on('mousedown.ajax-combobox',function(e) {
				if ($(document.body).data(constants.objStatusKey)) $(document.body).data(constants.objStatusKey,false);
				else {
					$('div.ac_container').each(function(){
						...
						if($(this).hasClass(d.css_class.container_open)) {
							...
							if(!$(d.elem.combo_input).val() && $(d.elem.hidden).val()){
							...
							if ($('li', $(d.elem.results)).size() > 0) {
								if(d.option.auto_fill_result) {//打开自动内容填充功能
									//若已有选中项目，则直接隐藏列表
									if ($('li.ac_selected', $(d.elem.results)).size() > 0) {
										d._hideResults(d);
									}else if($('li.ac_over', $(d.elem.results)).size() > 0){
										//若控件已有选中的值，则忽略高亮的项目
		...
		_getShortURL: function(self) {
			...
			$.ajax({
				...
				success: function(json) {
					// 元URLと短縮URLを順番に置換する
					var i = 0;
					var result = text.replace(self.option.shorten_reg,
					function() {
		...
		_scrollWindow: function(self, enforce) {
			...
			if (self.option.sub_info) {
				...
				target_size = $(dl).height() + parseInt($(dl).css('border-top-width')) + parseInt($(dl).css('border-bottom-width'));
			} else {
		...
		_enableButtonShort: function(self) {
			$(self.option.shorten_btn).removeClass(self.css_class.btn_short_off).removeAttr('disabled');
		},
		...
		_disableButtonShort: function(self) {
			$(self.option.shorten_btn).addClass(self.css_class.btn_short_off).attr('disabled', 'disabled');
		},
		...
		_checkValue: function(self) {
			...
			} else if (self.option.plugin_type == 'textarea' && $(self.elem.result_area).is(':visible')) {
				//textareaで、現在のタグから別のタグへ移動していないか検査する			
				var new_tag = self._findTag(self, now_value);
				if (!new_tag) self._hideResults(self);
				else if (new_tag.str != self.prop.tag.str || new_tag.pos_left != self.prop.tag.pos_left) {
		...
		_processKey: function(self, e) {
			if (($.inArray(e.keyCode, [27, 38, 40, 9]) > -1 && $(self.elem.result_area).is(':visible')) || ($.inArray(e.keyCode, [37, 39, 13, 9]) > -1 && self._getCurrentLine(self)) || (e.keyCode == 40 && self.option.plugin_type != 'textarea')) {
				e.preventDefault();
				e.stopPropagation();
				e.cancelBubble = true;
				e.returnValue = false;
		...
		_suggest: function(self) {
			...
			self._setLoading(self);	$(self.elem.sub_info).children('dl').hide(); // サブ情報消去
					...
					for (i = 0; i < json.cnt_page; i++) {
						...
						for (var key in json.originalResult[i]) {
							...
							if (key == self.option.field) {
								...
								if (self.option.show_field !== '' && $.inArray('*', self.option.show_field) == -1 && $.inArray(key, self.option.show_field) == -1) {
									continue;
								} else {
									json.subinfo[i][key] = json.originalResult[i][key];
								}
							}
						}
					}
		...
		_prepareResults: function(self, json, q_word, which_page_num) {
			...
			if (self.prop.is_paging === false) self._setCssFocusedInput(self);
				...
				//修复如果有选择项，则高亮选中项，而不是高亮第一项
				var liSelected = $('li.ac_selected', $(self.elem.results));
		...
		_displayResults: function(self, json, is_query) {
			...
			for (var i = 0; i < arr_candidate.length; i++) {
				...
				var list = $('<li>').html(itemText) // XSS対策
				.attr({
					pkey: arr_primary_key[i],
					title: itemText
				});

				if (self.option.plugin_type != 'textarea' && arr_primary_key[i] == $(self.elem.hidden).val()) {
			...
			$(self.elem.result_area).show(1,function(){
			    ...
			    */
				...
				if ($(self.elem.container).css('position') == 'static') {
					...
					var offset = $(self.elem.combo_input).offset();
					var p = $(self.elem.combo_input)[0].clientTop;
					...
					var left = (offset.left + listWidth) > browserWidth ? -(listWidth - $(self.elem.combo_input).outerWidth()) : 0;
					//控件在当前可视区域中的高度
					var screenTop = offset.top - $(window).scrollTop();
					var top = 0;
					if((screenTop + $(self.elem.combo_input).outerHeight() + listHeight) > browserHeight){
						top = -listHeight;
						$(self.elem.result_area).removeClass('shadowUp shadowDown').addClass('shadowUp');
					}else{
						top = $(self.elem.combo_input).outerHeight();
		...
		_selectCurrentLine: function(self, is_enter_key) {
			...
			if (current) {
				if (self.option.plugin_type != 'textarea') {
					...
					ctext = $(current).text();
					// 左側空白の補完
					if (self.option.tags[self.prop.tag.type].space[0] && !left.match(self.option.tags[self.prop.tag.type].pattern.space_left)) {
						p_len = self.option.tags[self.prop.tag.type].pattern.left.length;
						l_len = left.length;
						left = left.substring(0, (l_len - p_len)) + ' ' + left.substring((l_len - p_len));
					}
					// 右側カッコの補完
					...
					// 右側空白の補完
					if (self.option.tags[self.prop.tag.type].space[1] && !right.match(self.option.tags[self.prop.tag.type].pattern.space_right)) {
						p_len = self.option.tags[self.prop.tag.type].pattern.right.length;
						right = right.substring(0, p_len) + ' ' + right.substring(p_len);
					}
