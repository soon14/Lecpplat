/* [ ---- Gebo Admin Panel - common ---- ] */
    ...
    gebo_sidebar = {
		...
		make_active: function() {
			...
			if(thisHeading && thisHeading.length > 0) $(thisHeading).addClass('sdb_h_active');
		},
        make_scroll: function() {
            //antiScroll = $('.antiScroll').antiscroll().data('antiscroll');
        	$('div.antiscroll-content').slimscroll({
                height: '100%',
                opacity : 0.3,
                wheelStep : 5
            });
        ...
        update_scroll: function() {
			if($('.antiScroll').length) {
				if( $(window).width() > 979 ){
					var topbar = $('div.navbar-fixed-top').height();
					var mainPageBar = $('div.sidetit').height();
					$('.antiscroll-inner,.antiscroll-content').height($(window).height() - topbar - mainPageBar);
				} else {
	...
	gebo_external_links = {
		init: function() {
			...
			$("#side_accordion a[target='_blank']").each(function() {
